...
const MealParseLoading: React.FC<MealParseLoadingProps> = ({ transcript, exiting = false }) => {
  ...
  useEffect(() => {
    ...
    const revealTimer = window.setTimeout(() => setBreakdownReady(true), 280);
    const growTimers = [
      window.setTimeout(() => setDiscoveredRows(2), 900),
      window.setTimeout(() => setDiscoveredRows(3), 1650),
    ];
  ...
  if (!hasTranscript) {
    return (
      <div className="sahha-breakdown sahha-breakdown--wait" aria-live="polite" aria-busy="true">
        ...
        <p className="sahha-breakdown__stage">{getParseStageLabel(null, 'wait')}</p>
        <p className="sahha-breakdown__stage-sub">{getParseLoadingSublabel(null)}</p>
